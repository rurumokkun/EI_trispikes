...
const	int		INTERACT_TIME	=1000;					//グループ間相互作用時間
...
const	int		maxE=810;
	...
	w	float	sm = 10.0;		// maximal synaptic strength		
...
const int N_firings_max=100*N;	// upper limit on the number of fired neurons per sec		//発火数の上限: 変えていいのか？論文を確認すべき 9:1は必ず超える
...
int               Ne[Ngr][9]={{850,840,830,820,810,800,700,600,500},     {850,840,830,820,810,800,700,600,500},     {850,840,830,820,810,800,700,600,500}};

int                 Ni[Ngr][9];   //1/20実験 真ん中変える
	...
	//グループ内のsd更新の後、グループ間のsd更新		///	
			...
			{ 
				...
				//if(gr==0&&agr==0||gr==1||gr==Ngr-1&&agr==1)     //グループ1,3間は未接続
  				while(t-firings[gr][--z[gr]][0] <D+10 && z[gr]!=0)
				{
					if(t-firings[gr][z[gr]][0]>=10&&firings[gr][z[gr]][1]<Ne[gr][num])

					for(j=0; j< interM; j++)
    					{
					     if(t-firings[gr][z[gr]][0]==interpost_delays[gr][agr][firings[gr][z[gr]][1]][j])
                                      		{i=interpost[gr][agr][firings[gr][z[gr]][1]][j];
				      		 I[grandagrtoogr(gr,agr)][i]+=interpost_s[gr][agr][firings[gr][z[gr]][1]][j];
				      		 
					    	 interpost_sd[gr][agr][firings[gr][z[gr]][1]][j]-=inter_LTD[grandagrtoogr(gr,agr)][i];
						 }
					}
				}
			}		
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                        ...
                        if(sec>=SELF_SPIKE_TIME &&gr==Ngr-1)for(gr=0;gr<Ngr;gr++)for (i=0;i<N;i++)
			{
				...
				inter_LTP[gr][i][t+D+1]=0.95*inter_LTP[gr][i][t+D];			//0.95=exp(-1/20)
				inter_LTD[gr][i]*=0.95;
			}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		cout << "sec=" << sec << ", firing rate=" << float(N_firings[gr])/N << ", グループ1=\t" << Ne[0][num]/100<<":"<<Ni[0][num]/100  <<",グループ2=\t" << Ne[1][num]/100<<":"<<Ni[1][num]/100<<",グループ3=\t" << Ne[2][num]/100<<":"<<Ni[2][num]/100<<"\n";	//発火数を出力
		...
		///グループ間結合重み更新///
		if(sec>=SELF_SPIKE_TIME && sec<SIM_TIME-200 ){
		for (i=0;i<Ne[gr][num];i++)
		for(agr=0;agr<Ngr-1;agr++)
                ...
                if(sec==0||sec==1000||sec==1999)
		{
			...
			fp_s_3=fopen(fname_s_3,"w");
			 ...
			 for(i=0;i<Ne[0][num];i++)
			  for(j=0;j<interM;j++){
				fprintf(fp_interpost_s_1,"%f\n",interpost_s[0][0][i][j]);
			       }
			for(i=0;i<Ne[1][num];i++)
			  for(j=0;j<interM;j++){
				fprintf(fp_interpost_s_2,"%f\n",interpost_s[1][1][i][j]);
			 
			  }
			for(i=0;i<Ne[1][num];i++)
			  for(j=0;j<interM;j++){
				fprintf(fp_interpost_s_3,"%f\n",interpost_s[1][0][i][j]);
			       }
			for(i=0;i<Ne[2][num];i++)
			  for(j=0;j<interM;j++){
				fprintf(fp_interpost_s_4,"%f\n",interpost_s[2][1][i][j]);
			 
			  }
			for(i=0;i<Ne[2][num];i++)
			  for(j=0;j<interM;j++){
				fprintf(fp_interpost_s_5,"%f\n",interpost_s[2][0][i][j]);
			       }
			for(i=0;i<Ne[0][num];i++)
			  for(j=0;j<interM;j++){
				fprintf(fp_interpost_s_2,"%f\n",interpost_s[0][1][i][j]);
	                ...
	                for(i=0;i<Ne[0][num];i++)
			  for(j=0;j<M;j++){
				fprintf(fp_s_1,"%f\n",s[/*gr=*/0][i][j]);	//クッソ長いファイル注意!
			  }
			
		       for(i=0;i<Ne[1][num];i++)
			  for(j=0;j<M;j++){
				fprintf(fp_s_2,"%f\n",s[/*gr=*/1/*=Ngr-1*/][i][j]);	//クッソ長いファイル注意!
			  }

			for(i=0;i<Ne[2][num];i++)
			  for(j=0;j<M;j++){
				fprintf(fp_s_3,"%f\n",s[/*gr=*/2/*=Ngr-1*/][i][j]);	//クッソ長いファイル注意!
			  }
